...
extern user_upload_timer_t user_upload_timer;
...
void user_upstream_thread ( mico_thread_arg_t arg ) {
  ...
  while ( 1 ) {
    ...
    f_0x41_04_cmd() ;
    mico_thread_sleep ( 2 );
//    if(IsStoreData == 1 || Is_First_Upload_Sucess == 0)
//    {
...
OSStatus upload_dev_data ( unsigned char *recv_0x41_buf, unsigned char *recv_0x45_buf, mico_Context_t *mico_context ) {
  ...
  //定时时间
  user_upload_timer.type = ( int ) recv_0x45_buf[10];
  json_object_object_add ( send_json_object, "TMD", json_object_new_int ( (int)user_upload_timer.type ) );
  //set time
  user_upload_timer.set_time =  ( int ) recv_0x45_buf[11] * 60 + recv_0x45_buf[12];
  json_object_object_add ( send_json_object, "TST", json_object_new_int ( (int)user_upload_timer.set_time ) );
  //left time
  user_upload_timer.left_time =  ( int ) recv_0x45_buf[13] * 60 + recv_0x45_buf[14];
  json_object_object_add ( send_json_object, "TLT", json_object_new_int ( (int)user_upload_timer.left_time ) );
