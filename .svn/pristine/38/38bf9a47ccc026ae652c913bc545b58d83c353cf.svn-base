...
char order_num[41];
char ccid[40];
static uint8_t is_disabled_cmd_echo = 0;
...
void sim7600_reset_pin_init(void)
{
    ...
    RCC_APB2PeriphClockCmd(SIM7600_RESET_GPIO_CLK, ENABLE);
	
    GPIO_InitStruchture.GPIO_Pin = SIM7600_RESET_PIN;
    GPIO_InitStruchture.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_InitStruchture.GPIO_Speed = GPIO_Speed_50MHz;
	
    GPIO_Init(SIM7600_RESET_PORT, &GPIO_InitStruchture);
...
{  
    ...
    sim7600_reset_pin_low();
    OSTimeDlyHMSM(0,0,1,0);
    sim76000_reset_pin_high();
...
void sim7600_powerkey_off(void)
{
    ...
    SIM7600_LOG("power off");  
}

void sim7600_power_restart(void)
{  
    SIM7600_LOG("SIM7600 power restart");
...
int sim7600_wait_response(char *rx_buff,uint16_t *index)
{
    ...
    //超时 没有返回数据
    if(nowtime_ms >= (sim7600_cmd_info.cmd_send_time + SIM7600_CMD_TIMEOUT_TIME))
    {
        sim7600_cmd_info.cmd_result = CMD_TIMEOUT;
      
        SIM7600_LOG("cmd:%s timeout",sim7600_cmd_info.cmd);
    ...
    if(strstr(rx_buff,END_ERROR) != NULL)
    {
        ...
        {
            SIM7600_LOG("cmd:%s error",sim7600_cmd_info.cmd);         
        }
        else
        {
            SIM7600_LOG("send string error");
        }
    ...
    {
        //发送字符串时返回OK或者ERROR
        if(strstr(rx_buff,sim7600_cmd_info.end_flag) != NULL)
        {
    ...
    else if(sim7600_cmd_info.type == CMD_TYPE)
    {
        ...
        {            
            ...
            SIM7600_LOG("cmd:%s success",sim7600_cmd_info.cmd);
        ...
        {   
            ...
            else if(strcmp(sim7600_cmd_info.cmd,SIM7600_CSQ_CMD) == 0)
            {
                ...
                {                   
                    ...
                    strncpy(sim7600_cmd_info.sendtring,(pos + 2),i);
                    signal_quality = atoi(sim7600_cmd_info.sendtring);
                    
                    SIM7600_LOG("signal_quality = %d",signal_quality);
                }              
            }
            ...
            else if(strcmp(sim7600_cmd_info.cmd,SIM7600_CICCID_CMD) == 0)
            {
                ...
                pos = strstr(rx_buff,SIM7600_CICCID_START_FLAG);
                if(pos == NULL)
                {
                    SIM7600_LOG("not find ccid start flag");
                    SIM7600_LOG("CCID:%s",ccid);
                    empty_rx_buff(rx_buff,index);
                    goto exit;;
                }
                strncpy(ccid,pos + strlen(SIM7600_CICCID_START_FLAG) + 1,SIM7600_CCID_LEN);
              
                SIM7600_LOG("index = %d",*index);
            ...
            else if(strcmp(sim7600_cmd_info.cmd,SIM7600_HTTPSSEND_NZ_CMD) == 0)
            {                       
                //printf("SIM7600_HTTPSSEND_CMD OK\r\n");
              
                https_step += 1;
            }
            else if(strcmp(sim7600_cmd_info.cmd,SIM7600_HTTPS_SEND_REQUEST_CMD) == 0)
            {                       
            ...
            else if(strcmp(sim7600_cmd_info.cmd,SIM7600_HTTPSSEND_Z_CMD) == 0)
            {                       
                //printf("SIM7600_HTTPSSEND_CMD OK\r\n");              
                https_step += 1;             
            }
            ...
            { 
                ...
                {
                    //消息队列满了，push一个出来
                    SIM7600_LOG("upload data queue full,push one msg");
                ...
                if(err != OS_ERR_NONE)
                {
                    SIM7600_LOG("push upload data msg error");
                    if(https_req_res.upload_res.http_body != NULL)
                    {
            ...
            else if(strcmp(sim7600_cmd_info.cmd,SIM7600_MQTT_SEND_TOPIC_CMD) == 0)
            {
                ...
                if(err != OS_ERR_NONE)
                {
                    SIM7600_LOG("push msg to mqtt_action_res_queue fail");
                }                                                     
            }          
        }
...
//    }
    ...
    { 
        ...
        else if(strcmp(cmd,SIM7600_DISABLE_CMD_ECHO_CMD) == 0)
        {
            ...
            strcpy(sim7600_cmd_info.end_flag,SIM7600_DISABLE_CMD_ECHO_RES);
        }
        ...
        else if(strcmp(cmd,SIM7600_HTTPS_START_CMD) == 0)     //https
        {
            ...
            strcpy(sim7600_cmd_info.end_flag,SIM7600_HTTPS_START_RES);
        }
        ...
        else if(strcmp(cmd,SIM7600_HTTPSSEND_NZ_CMD) == 0)   
        {          
            sprintf(pbuff,"%s=%d%s",SIM7600_HTTPSSEND_CMD,http_requeset_length,SIM7600_CMD_END_FLAG);
            strcpy(sim7600_cmd_info.end_flag,SIM7600_HTTPSSEND_NZ_RES);                
        }
        else if(strcmp(cmd,SIM7600_HTTPS_SEND_REQUEST_CMD) == 0)   
        {
            //SIM7600_HTTPS_SEND_REQUEST_CMD命令发送的是http请求的字符串，并不是真正的命令
            strcpy(sim7600_cmd_info.end_flag,SIM7600_HTTPS_SEND_REQUEST_RES);                
        }
        else if(strcmp(cmd,SIM7600_HTTPSSEND_Z_CMD) == 0)   
        {          
            sprintf(pbuff,"%s%s",SIM7600_HTTPSSEND_CMD,SIM7600_CMD_END_FLAG);
            strcpy(sim7600_cmd_info.end_flag,SIM7600_HTTPSSEND_Z_RES);                
        }
        ...
        else if(strcmp(cmd,SIM7600_HTTPSSTOP_CMD) == 0)   
        {
            ...
            strcpy(sim7600_cmd_info.end_flag,SIM7600_HTTPSSTOP_RES);
        ...
        else if(strcmp(cmd,SIM7600_MQTT_START_CMD) == 0)         //mqtt
        {
        ...
        {
            sprintf(pbuff,"%s=0,\"%s\"%s",SIM7600_MQTT_ACCQ_CMD,MQTT_CLIENT_ID,SIM7600_CMD_END_FLAG);
            strcpy(sim7600_cmd_info.end_flag,END_OK);
        }
        ...
        else if(strcmp(cmd,SIM7600_MQTT_CONNECT_CMD) == 0)
        {
            sprintf(pbuff,"%s=0,\"tcp://%s:%d\",%d,%d,\"%s\",\"%d\"%s",SIM7600_MQTT_CONNECT_CMD,MQTT_SERVER_ADDR,MQTT_PORT,\
                      SIM7600_MQTT_KEEPALIVE_TIME,SIM7600_MQTT_CLEAN_SESSION,device_id,mqtt_passowrd,SIM7600_CMD_END_FLAG);
            strcpy(sim7600_cmd_info.end_flag,END_OK);
        }
    ...
    if(type == CMD_TYPE && strcmp(cmd,SIM7600_HTTPS_SEND_REQUEST_CMD))
    {
    ...
    sim7600_cmd_info.cmd_send_time = OSTimeGet();
...
void sim7600_http_loop(void)
{
    ...
    if(sim7600_cmd_info.Is_wait_data == 1 || mqtt_action != 0 || is_https_action == 0)
    {
        return;
    }
    ...
    if(https_step == 0)
    {
        ...
        {
            SIM7600_LOG("http loop push msg fail");
            goto https_step0_error;
        }
        
        sim7600_action = https_req_res.upload_msg->action;
    ...
    {  
        ...
        if(https_req_res.header == NULL)
        {
            SIM7600_LOG("http_request malloc fail");
            goto error;
        }
        ...
        if(https_req_res.http_body == NULL)
        {
            SIM7600_LOG("http_body malloc fail");
            goto error;
        }
        ...
        sprintf(https_req_res.header + strlen(https_req_res.header), "Content-Length: %d\r\n", http_body_length - 2);
        ...
        {
            strcat(https_req_res.header,SIM7600_CMD_END_FLAG);
            http_request_length = strlen(https_req_res.header) + http_body_length;
        }
           
          
        sim7600_cmd_info.type = CMD_TYPE;       
        strcpy(sim7600_cmd_info.cmd,SIM7600_HTTPSSEND_NZ_CMD); 
    }
    else if(https_step == 3)
    {
        //发送的http request数据量较大，必须分几次发送，否则模块会异常。先发送http header,然后发送jwt,最后发送http body,且中间要延时
        uart_sim7600_sendbytes((uint8_t *)https_req_res.header,strlen(https_req_res.header));
        OSTimeDlyHMSM(0,0,0,2); 
        SIM7600_LOG("%s",https_req_res.header);
        if(https_req_res.upload_msg->is_jwt == 1)
        {
            uart_sim7600_sendbytes((uint8_t *)https_req_res.jwt,strlen(https_req_res.jwt));
            OSTimeDlyHMSM(0,0,0,2); 
            SIM7600_LOG("%s",https_req_res.jwt);
        }
        
      
        sim7600_cmd_info.type = CMD_TYPE;       
        strcpy(sim7600_cmd_info.cmd,SIM7600_HTTPS_SEND_REQUEST_CMD);
    }
//    else if(https_step == 4)
//    {
//        sim7600_cmd_info.type = CMD_TYPE;       
//        strcpy(sim7600_cmd_info.cmd,SIM7600_HTTPSSEND_Z_CMD);
//    }
    else if(https_step == 4)
    {
        sim7600_cmd_info.type = CMD_TYPE;       
    ...
    //SIM7600_Send_data(sim7600_cmd_info.type,sim7600_cmd_info.cmd,g_http_requeset,http_request_length);
    sim7600_send_data(sim7600_cmd_info.type,sim7600_cmd_info.cmd,https_req_res.http_body,http_request_length);
...
error:
    ...
    SIM7600_LOG("http loop malloc fail");
    sim7600_http_close();
    return;
...
{  
    ...
    {
        //SIM7600_LOG("mqtt_action_cmd_queue not empty");
        mqtt_action_cmd = OSQPend(mqtt_action_cmd_queue,10,&err);
        if(err != OS_ERR_NONE || mqtt_action_cmd == NULL)
        {
            SIM7600_LOG("get mqtt cmd fail");
            return;
        }
        //SIM7600_LOG("mqtt_action_cmd:%p",mqtt_action_cmd);
        
        SIM7600_LOG("mqtt cmd:%d",mqtt_action_cmd->cmd);
        if(mqtt_action_cmd->cmd == MQTT_INIT_ACTION)
        {
            SIM7600_LOG("scan MQTT_INIT_ACTION");
            mqtt_action = MQTT_INIT_ACTION; 
            mqtt_init_step = 0;           
        }
        else if(mqtt_action_cmd->cmd == MQTT_GET_CONNECT_STATUS_ACTION)
        {
            SIM7600_LOG("scan MQTT_GET_CONNECT_STATUS_ACTION");
...
void sim7600_mqtt_loop(void)
{
    ...
    if(sim7600_cmd_info.Is_wait_data == 1 || is_https_action == 1 || mqtt_action == 0)
    {
        return;
    }
    ...
    if(mqtt_action == MQTT_INIT_ACTION)
    {
        ...
        {
            //在MQTT连接成功后，发送SIM7600_MQTT_SUB_CMD前必须延时，否则SIM7600_MQTT_SUB_CMD命令会失败，具体延时多少时间没有测试，延时1秒正常。
            OSTimeDlyHMSM(0,0,1,0); 
            sim7600_cmd_info.type = CMD_TYPE;       
...
//        {
//            sim7600_cmd_info.type = CMD_TYPE; 
//            strcpy(sim7600_cmd_info.cmd,SIM7600_MQTT_GET_STATUS_CMD);  
//            sim7600_send_data(sim7600_cmd_info.type,sim7600_cmd_info.cmd,NULL,0);  
//          
...
void sim7600_get_mqtt_message(char *rx_buff,uint16_t *index)
{
    ...
    mqtt_start_pos = strstr(rx_buff,SIM7600_MQTT_RX_START_FLAG);
    if(mqtt_start_pos != NULL)
    {       
        mqtt_stop_pos = strstr(mqtt_start_pos,SIM7600_MQTT_RX_STOP_FLAG);
        if(mqtt_stop_pos != NULL)
        {
            payload_pos = strstr(mqtt_start_pos,SIM7600_MQTT_RX_PAYLOAD_FLAG);
            if(payload_pos != NULL)
            {       
                for(pos = payload_pos;pos < mqtt_stop_pos;pos++)
                {
                    if(*pos == '\r' && *(pos + 1) == '\n')
                    {
                        break;
                ...
                if(pos < (mqtt_stop_pos - 3) && *(pos + 2) == '{')
                {
                    payload_start_pos = pos + 2;
                    payload_end_pos = mqtt_stop_pos - 3;
                ...
                {
                    //此条mqtt消息太长，废弃掉
                    SIM7600_LOG("mqtt pyaload too long");
                ...
                {
                    SIM7600_LOG("mqtt queue full,push one message");
                    payload = OSQPend(mqtt_message_queue,10,&err);
                    if(err != OS_ERR_NONE)
                    {
                        SIM7600_LOG("mqtt queue full,push one message fail\r\n");
                        return;
                    }
                ...
                payload = mymalloc(SIM7600_PAYLAOD_BUFF_SIZE);
                if(payload == NULL)
                {
                    goto exit;
                }
                memset(payload,0,SIM7600_PAYLAOD_BUFF_SIZE);
                
                strncpy(payload,payload_start_pos,payload_length);
                
                //SIM7600_LOG("payload_length=%d",payload_length);
                ...
                if((mqtt_stop_pos + 2) == (rx_buff + *index) && sim7600_cmd_info.Is_wait_data == 0)
                {
                    empty_rx_buff(rx_buff,index);
                    goto exit;
                }
...
void sim7600_mqtt_close(void)
{
    ...
    memset(sim7600_cmd_info.cmd,0,sizeof(sim7600_cmd_info.cmd));   
    sim7600_cmd_info.type = CMD_TYPE;      
    strcpy(sim7600_cmd_info.cmd,SIM7600_MQTT_DISCONNECT_CMD);
    sim7600_send_data(sim7600_cmd_info.type,sim7600_cmd_info.cmd,NULL,0);   
    OSTimeDlyHMSM(0,0,1,0);   
...
//    }
    ...
    {
        //消息队列满了，push一个出来
        SIM7600_LOG("upload upload queue full,push one msg");
    ...
    if(err != OS_ERR_NONE)
    {
        ...
        {
            SIM7600_LOG("upload data timeout");
            res = UPLOAD_TIMEOUT;
        }
        else
        {
            SIM7600_LOG("push from upload res queue err:%d",err);
            res = err;
        }
    ...
    {
        SIM7600_LOG("upload_res_msg == NULL");
        res = UPLOAD_RES_NULL;
        goto exit;
    }
...
https_start:
    ...
    if(sim7600_register_net.is_sim7600_registerd == 0 || Is_Get_CCID == 0)
    {
        res = MODULE_NOT_UP;
        try_times = HTTPS_MAX_TRY_TIMES;
        goto exit;
    ...
    {
        //消息队列满了，push一个出来
        SIM7600_LOG("activate upload queue full,push one");
        temp_msg = OSQPend(upload_data_queue,10,&err);
        if(err != OS_ERR_NONE)
        {
            res = UPLAOD_REQ_QUEUE_ERR;
    ...
    SIM7600_LOG("wait for activate");
    upload_res_msg = OSQPend(upload_res_queue,WAIT_HTTP_RES_MAX_TIME,&err);
    if(err != OS_ERR_NONE)
    {
        if(err == OS_ERR_TIMEOUT)
        {
            SIM7600_LOG("device_activate timeout");
            res = UPLOAD_TIMEOUT;
        }
        else
        {
    ...
    if(root == NULL)
    {
        SIM7600_LOG("json string formot error");
        res = JSON_ITEM_ERR;
        goto exit;
    }
    ...
    if(meta_item == NULL)
    {
        SIM7600_LOG("not find meta_item");
        res = JSON_ITEM_ERR;
        goto exit;
    }
    ...
    if(code_item == NULL)
    {
        SIM7600_LOG("not find code item");
        res = JSON_ITEM_ERR;
        goto exit;
    }
    ...
    if(data_item == NULL)
    {
        SIM7600_LOG("not find data_item");
        res = JSON_ITEM_ERR;
        goto exit;
    }
    ...
    if(code == 0)
    {
        ...
        SIM7600_LOG("device_id:%s",device_id);
    }
    else
    {
        SIM7600_LOG("code = %d",code);
    }
...
https_start:  
    ...
    if(sim7600_register_net.is_sim7600_registerd == 0 || Is_Get_CCID == 0 || Is_device_activate == 0)
    {
        res = MODULE_NOT_UP;
        try_times = HTTPS_MAX_TRY_TIMES;
        goto exit;
    ...
    {
        //消息队列满了，push一个出来
        SIM7600_LOG("token upload queue full,push one msg");
        temp_msg = OSQPend(upload_data_queue,10,&err);
        if(err != OS_ERR_NONE)
        {
            res = UPLAOD_REQ_QUEUE_ERR;
    ...
    SIM7600_LOG("wait for get token");
    upload_res_msg = OSQPend(upload_res_queue,WAIT_HTTP_RES_MAX_TIME,&err);
    if(err != OS_ERR_NONE)
    {
        if(err == OS_ERR_TIMEOUT)
        {
            SIM7600_LOG("device get token timeout");
            res = UPLOAD_TIMEOUT;
        }
        else
        {
...
https_start:  
    ...
    {
        //消息队列满了，push一个出来
        SIM7600_LOG("ota check upload queue full,push one msg");
        temp_msg = OSQPend(upload_data_queue,10,&err);
        if(err != OS_ERR_NONE)
        {
            res = UPLAOD_REQ_QUEUE_ERR;
    ...
    SIM7600_LOG("wait for ota check");
    upload_res_msg = OSQPend(upload_res_queue,WAIT_HTTP_RES_MAX_TIME,&err);
    if(err != OS_ERR_NONE)
    {
        if(err == OS_ERR_TIMEOUT)
        {
            SIM7600_LOG("device ota check timeout");
            res = UPLOAD_TIMEOUT;
        }
        else
        {
    ...
    {
        SIM7600_LOG("not find ota_status_item");
        res = JSON_ITEM_ERR;
        goto exit;
    }
    ...
    {
        SIM7600_LOG("not find ota_file_item");
        res = JSON_ITEM_ERR;
        goto exit;
    }
...
mqtt_init_start:  
    ...
    {
        //消息队列满了，pull一个出来
        SIM7600_LOG("mqtt init queue full,pull one msg");
    ...
    if(err != OS_ERR_NONE)
    {
        SIM7600_LOG("push mqtt init msg error");
        res = MQTT_CMD_QUEUE_ERR;
        goto exit;
    }
    
    SIM7600_LOG("wait for mqtt init");
    mqtt_action_res_msg = OSQPend(mqtt_action_res_queue,WAIT_MQTT_INIT_RES_MAX_TIME,&err);
    if(err != OS_ERR_NONE)
    {
        if(err == OS_ERR_TIMEOUT)
        {
            SIM7600_LOG("mqtt init timeout");
            res = MQTT_CMD_RES_TIMEOUT;
        }
        else
        {
            SIM7600_LOG("push from mqtt cmd queue err:%d",err);
            res = err;
        }
    ...
    {
        SIM7600_LOG("mqtt_action_res_msg == NULL");
    ...
    {
        SIM7600_LOG("mqtt init res cmd err");
        goto exit;
    }
    
    if(mqtt_action_res_msg->result != MQTT_ACTION_SUCCESS)
    {
        SIM7600_LOG("mqtt init cmd fail %d times",(try_times + 1));
        goto exit;
    }
...
uint8_t get_mqtt_connect_status_event(uint8_t *mqtt_status)
{
    ...
    if(is_mqtt_inited == 0)
    {
        SIM7600_LOG("mqtt not init,return");
        *mqtt_status = 0;
        res = MQTT_ACTION_SUCCESS;
        return res;
    }
...
//    }
    ...
    {
        //消息队列满了，pull一个出来
        SIM7600_LOG("mqtt queue full,pull one msg");
        temp_msg = OSQPend(mqtt_action_cmd_queue,10,&err);
        if(err != OS_ERR_NONE)
        {
            res = MQTT_CMD_QUEUE_ERR;
    ...
    {
        //消息队列满了，pull一个出来
        SIM7600_LOG("mqtt_action_res_queue full,pull one msg");
    ...
    //SIM7600_LOG("push mqtt cmd:%d",mqtt_action_cmd_msg.cmd);
    //SIM7600_LOG("mqtt_action_cmd_msg:%p",&mqtt_action_cmd_msg);
    
    err = OSQPost(mqtt_action_cmd_queue,&mqtt_action_cmd_msg);
    if(err != OS_ERR_NONE)
    {
        SIM7600_LOG("push get mqtt status msg error");
        res = MQTT_CMD_QUEUE_ERR;
        goto exit;
    }
    
    SIM7600_LOG("wait for get mqtt status");
    mqtt_action_res_msg = OSQPend(mqtt_action_res_queue,WAIT_MQTT_INIT_RES_MAX_TIME,&err);
    if(err != OS_ERR_NONE)
    {
        if(err == OS_ERR_TIMEOUT)
        {
            SIM7600_LOG("get mqtt status timeout");
            res = MQTT_CMD_RES_TIMEOUT;
        }
        else
        {
    ...
    if(mqtt_action_res_msg->cmd != mqtt_action_cmd_msg.cmd)
    {
        SIM7600_LOG("get mqtt status res cmd err");
        goto exit;
    }
    
    if(mqtt_action_res_msg->result != MQTT_ACTION_SUCCESS)
    {
        SIM7600_LOG("get mqtt status cmd fail %d times",(try_times + 1));
        goto exit;
    }
    
    
    SIM7600_LOG("mqtt_action_res_msg->status:%d",mqtt_action_res_msg->status);
...
restart:    
    sim7600_power_restart(); 
    time_count = 0;
    event_fail_times = 0;
  
disable_echo:
    if(sim7600_register_net.is_sim7600_registerd == 1)
    {
        sim7600_disnable_command_echo();
        OSTimeDlyHMSM(0,0,1,0);
        event_fail_times += 1;
        if(is_disabled_cmd_echo != 1)
        {
    ...
    else
    {
        OSTimeDlyHMSM(0,0,1,0); 
        time_count += 1;
        SIM7600_LOG("time_count:%d",time_count);
        if(time_count >= 60)
        {
...
get_ccid_label:
    ...
    {
        SIM7600_LOG("get ccid fail %d times",event_fail_times);
        event_fail_times += 1;
        if(event_fail_times >= 30)
        {
            goto restart;
...
//            if(res == MQTT_ACTION_SUCCESS)
//            {
//                SIM7600_LOG("get mqtt status:%d",mqtt_status);
//            }
//            else
//            {
//                SIM7600_LOG("get mqtt status fail");
//            }
//            
//            time_count = 0;
//        }
