...
linux32unamestr=`"${scriptdir}MiCoder/cmd/Linux32/uname" 2> /dev/null`
linux64unamestr=`"${scriptdir}MiCoder/cmd/Linux64/uname" 2> /dev/null`
osxunamestr=`"${scriptdir}MiCoder/cmd/OSX/uname"  2> /dev/null`
win32unamestr=`"${scriptdir}MiCoder/cmd/Win32/uname.exe" -o 2> /dev/null`

if [ "$linux32unamestr" == "Linux" ] || [ "$linux64unamestr" == "Linux" ]; then
# Linux
linuxuname64str=`"${scriptdir}MiCoder/cmd/Linux64/uname" -m 2> /dev/null`
if [ "$linuxuname64str" == "x86_64" ]; then

# Linux64
#echo Host is Linux64
"${scriptdir}MiCoder/cmd/Linux64/make" "$@" HOST_OS=Linux64

else

# Linux32
#echo Host is Linux32
"${scriptdir}MiCoder/cmd/Linux32/make" "$@" HOST_OS=Linux32
...
#OSX
#echo Host is OSX
"${scriptdir}MiCoder/cmd/OSX/make" "$@" HOST_OS=OSX

elif [ "${win32unamestr}" == "MinGW" ]; then

#MinGW / Cygwin
#echo Host is MinGW
"${scriptdir}MiCoder/cmd/Win32/make.exe" "$@" HOST_OS=Win32
