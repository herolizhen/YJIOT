...
namespace YJIOT.ShareAirAPP.Service
{
    ...
    public class APPConfig
    {
        ...
        public static int USE_PAYTIMEOUT
        {
            get
            {
                return int.Parse(ConfigurationManager.AppSettings["USE:PAYTIMEOUT"]);
            }
        }
        ...
        public static List<PayCondition> USE_PayCondition
        {
            get
            {
                List<PayCondition> pcs = new List<PayCondition>();

                string v = ConfigurationManager.AppSettings["USE:PayCondition"];
                try
                {
                    foreach (var item in v.Split('|'))
                    {
                        if (!string.IsNullOrEmpty(item))
                        {
                            ...
                            PayCondition pc = new PayCondition();
                            pc._Pay = int.Parse(tmps[0]);
                            pc._Count = int.Parse(tmps[1]);
                            pc._Time = int.Parse(tmps[2]);
                            pcs.Add(pc);
                        }
                    }
                }
        ...
        public static List<ConsumptionPackage> USE_ConsumptionPackage()
        {
            List<ConsumptionPackage> cps = new List<Models.ConsumptionPackage>();

            string vs = ConfigurationManager.AppSettings["USE:ConsumptionPackage"];
            try
            {
                foreach (var item in vs.Split('|'))
                {
                    ConsumptionPackage cp = new ConsumptionPackage();
                    cp.Packages = new List<ConsumptionPackage.Package>();
                    if (!string.IsNullOrEmpty(item))
                    {
                        string[] tmps = item.Split('$');
                        if (tmps.Length == 2)
                        {
                            cp.TypeKey = tmps[0];
                            string[] _tmps = tmps[1].Split('#');
                            foreach (var tmp in _tmps)
                            {
                                ConsumptionPackage.Package p = new ConsumptionPackage.Package();
                                var ts = tmp.Split(',');
                                p.Price = int.Parse(ts[0]);
                                p.ChipsText = ts[1];
                                p.TextKey = int.Parse(ts[2]);
                                cp.Packages.Add(p);
                            }
                        }
                    }
