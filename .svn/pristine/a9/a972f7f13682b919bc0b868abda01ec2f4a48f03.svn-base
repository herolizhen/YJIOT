...
void uart_recv_thread(mico_thread_arg_t arg)
{
  ...
  unsigned long wifi_led_check_nexttime = 0;
  const unsigned long wifi_led_check_time_interval = 5000;
  ...
  while(1) {
    ...
    if(wifi_led_check_nexttime > 0xF0000000 && nowtime <= 0xE0000000 )
    {
      //time overflow
      wifi_led_check_nexttime = nowtime;
    }
    if(nowtime >= wifi_led_check_nexttime)
    {
      if(nowtime >= (0xFFFFFFFF - wifi_led_check_time_interval) && wifi_led_check_nexttime <= wifi_led_check_time_interval)
      {
      ...
      else
      {
        ...
        wifi_led_check_nexttime = nowtime + wifi_led_check_time_interval;
      }
