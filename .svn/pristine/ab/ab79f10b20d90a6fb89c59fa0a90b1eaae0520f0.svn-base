...
void Task_Start(void *p_arg)
{
    ...
    {
        APP_LOG("creat upload_data_queue fail");
    }
    else
    {
        APP_LOG("creat upload_data_queue success");
    }
    
    upload_res_queue = OSQCreate(&upload_data_msg[0],UPLOAD_RES_NUM);
    if(upload_res_queue == NULL)
    {
        APP_LOG("creat upload_res_queue fail");
    }
    else
    {
        APP_LOG("creat upload_res_queue success");
    }

    mqtt_action_cmd_queue = OSQCreate(&mqtt_action_cmd_msg[0],MQTT_DATA_MESSAGE_NUM);
    if(mqtt_action_cmd_queue == NULL)
    {
        APP_LOG("creat mqtt_action_cmd_queue fail");
    }
    else
    {
        APP_LOG("creat mqtt_action_cmd_queue success");
    }
    
    mqtt_action_res_queue = OSQCreate(&mqtt_action_res_msg[0],MQTT_ACTION_RES_MESSAGE_NUM);
    if(mqtt_action_res_queue == NULL)
    {
        APP_LOG("creat mqtt_action_res_queue fail");
    }
    else
    {
        APP_LOG("creat mqtt_action_res_queue success");
    }
    
    mqtt_message_queue = OSQCreate(&mqtt_data_msg[0],MQTT_DATA_MESSAGE_NUM);
    if(mqtt_message_queue == NULL)
    {
        APP_LOG("creat mqtt_message_queue fail");
    }
    else
    {
        APP_LOG("creat mqtt_message_queue success");
    }
