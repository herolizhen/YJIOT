...
//            DEBUG_Uart_Sendbytes(debug_send_buff,mystrlen(debug_send_buff));
        ...
        else if(framebuffer[3] == 0x66)
        {
//            mymemset(debug_send_buff,0,30);
//            sprintf(debug_send_buff,"get 0x66 data\n");
//            DEBUG_Uart_Sendbytes(debug_send_buff,mystrlen(debug_send_buff));
        }
    }      
...
{
    unsigned char sendbuff[BUFF_SIZE_0x41] = {0};
    unsigned char i,j = 0;
    ...
    if(child_cmd <= 0x0A)
    {
        ...
        switch (child_cmd)
        {
          ...
          case 0x04:
            ...
            TM1820_LED_Control(child_cmd + 1,LED_ON);
            break;
          case 0x05:
            UV_Off();
            break;
...
void updata_cost_time(unsigned char *framebuff)
{
    ...
    if(framebuff[4] == 0x05)
    {
        if(framebuff[5] == 1)
        {
            //如果设备处于计费状态，而且设备还有剩余时间，是否把时间加一起
            if(cost_info.IsCostType == 1 && cost_info.IsHavetime == 1)
            {
                add_time = framebuff[6] * MONTH_TO_MIN + framebuff[7] * DAY_TO_MIN + framebuff[8] * HOUR_TO_MIN + framebuff[9];
                cost_info.lefttime += add_time;
                cost_info.left_time_month = cost_info.lefttime / MONTH_TO_MIN;
                temptime = cost_info.lefttime % MONTH_TO_MIN;
                cost_info.left_time_day = temptime / DAY_TO_MIN;
                temptime = temptime % DAY_TO_MIN;
                cost_info.left_time_hour = temptime / HOUR_TO_MIN;
                temptime = temptime % HOUR_TO_MIN;
                cost_info.left_time_min = temptime;              
            }
            else
            {
                if((framebuff[6] | framebuff[7] | framebuff[8] | framebuff[9]) == 0)
                {
                ...
                cost_info.left_time_month = framebuff[6];
                cost_info.left_time_day = framebuff[7];
                cost_info.left_time_hour = framebuff[8];
                cost_info.left_time_min = framebuff[9];
                cost_info.lefttime = cost_info.left_time_month * MONTH_TO_MIN + cost_info.left_time_day * DAY_TO_MIN + cost_info.left_time_hour * HOUR_TO_MIN + cost_info.left_time_min;
            }
        }
        else if(framebuff[5] == 0)
        {
            cost_info.IsCostType = 0;
            cost_info.IsHavetime = 0;
            cost_info.left_time_month = 0;
            cost_info.left_time_day = 0;
            cost_info.left_time_hour = 0;
            cost_info.left_time_min = 0;
