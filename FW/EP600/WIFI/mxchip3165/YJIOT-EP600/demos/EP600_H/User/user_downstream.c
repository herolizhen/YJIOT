      ...
      if (NULL != recv_json_object) {
        ...
        json_object_object_foreach(recv_json_object, key, val)
        {
          ...
          else
          {
            ...
            else if(!strcmp(key, "TMD")) {
              user_set_timer.type = key_value;
              
            }
            else if(!strcmp(key, "TST")) {
              user_set_timer.set_time = key_value;
...
//解析返回结果，此函数共享机使用，防止MQTT中断时收不到筹码，http上传数据服务器会把筹码和控制命令返回，在http中也可以处理筹码和控制命令
