...
extern user_set_timer_t user_set_timer;
...
volatile bool is_recv_0x41_data = false;
volatile bool is_recv_0x45_data = false;
//volatile bool is_modbus_busy_flag = false ;
volatile unsigned char is_modbus_busy_flag = false ;
...
OSStatus f_0x45_data ( unsigned char *recv_buf, mico_Context_t* mico_context ) {
  ...
  //user_modbus_log ( "get f_0x45_data" );
  
  if ( frame_comm_ct == 0xAF ) {
    
    if(DATA_LENGTH_0x45 < frame_size)
    {
      user_modbus_log ( "DATA_LENGTH_0x45 < frame_size!" );
      return -1;
    }
...
//  mico_rtos_lock_mutex( &stdio_tx_mutex );
//  printf("f_0x45_AF_data data:");
//  for(i = 0;i < sizeof(recv_0x45_data);i++)
//  {
//    printf("%x ",recv_0x45_data[i]);
//  }
//  printf("\n");
//  mico_rtos_unlock_mutex( &stdio_tx_mutex ); 
...
OSStatus f_0x49_data ( unsigned char *recv_buf, mico_Context_t* mico_context ) {
  ...
  }else if ( frame_comm_ct == 5 ) {
    err = f_0x49_05_data ( recv_buf );
...
//设置定时 f_0x49_05
OSStatus user_set_timer_cmd(int type,int time)
{
  OSStatus err = kUnknownErr;
  unsigned char command_to_send[] = {0x0A, 0x2A, 0x01, 0x49, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x23};
  command_to_send[2] = SENSOR_ADDRESS;
  
  command_to_send[5] = (unsigned char)type;
  command_to_send[6] = (unsigned char)(time / 60);
  command_to_send[7] = (unsigned char)(time % 60);
  
  user_modbus_log("user timer data send,type:%d,time:%d",(unsigned char)type,time);
  err = user_uart_send ( ( unsigned char* ) command_to_send, sizeof(command_to_send));
  require_noerr_action ( err, exit, user_modbus_log ( "ERROR: user_set_timer_cmd exit with err=%d", err ) );
...
//MCU给WIFI发送的定时确认
OSStatus f_0x49_05_data ( unsigned char *recv_buf ) {  
  OSStatus err = kUnknownErr;
  int time = 0;
  
  time = (int)(recv_buf[6]) * 60 + recv_buf[7];
  if(recv_buf[5] == (unsigned char)user_set_timer.type && time == user_set_timer.set_time)
  {
    //MCU成功收到定时信息
    user_set_timer.type = -1;
    user_set_timer.set_time = -1;
      
    user_modbus_log("mcu receive time data sucess");
  }
  else
  {
    user_modbus_log("mcu receive time data fail");
  } 
  return err;  
}
