...
{  
    ...
    return (used*100)/(memtblsize[SRAMIN]);  
}

u32 mem_free_size(void)
{
    u32 used=0;  
    u32 i;  
    for(i=0;i<memtblsize[SRAMIN];i++)  
    ...
    return ((memtblsize[SRAMIN] - used) * 32);
}
...
{  
	u32 offset;  
    if(ptr==NULL)return;//地址为0.  
 	offset=(u32)ptr-(u32)mallco_dev.membase[SRAMIN];  
    mem_free(SRAMIN,offset);//释放内存     
} 
...
{  
  u32 offset;  									      
	offset=mem_malloc(SRAMIN,size);  	   				   
  if(offset==0XFFFFFFFF)return NULL;  
  else return (void*)((u32)mallco_dev.membase[SRAMIN]+offset);  
}  
...
{  
    ...
    {  									   
	    mymemcpy((void*)((u32)mallco_dev.membase[SRAMIN]+offset),ptr,size);	//拷贝旧内存内容到新内存   
        myfree(ptr);  											  		//释放旧内存
        return (void*)((u32)mallco_dev.membase[SRAMIN]+offset);  				//返回新内存首地址
    }  
}
