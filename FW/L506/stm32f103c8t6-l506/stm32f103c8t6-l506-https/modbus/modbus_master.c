...
#include  "ucos_ii.h" 
 ...
 static const uint8_t ku8MBInvalidCRC                 = 0xE3;
    ...
    //static const uint8_t ku8MaxBufferSize                = 47;   ///< size of response/transmit buffers       
  ...
  // Modbus function codes for bit access
    ...
    //lixichao modify,user set this value via pc software
...
uint8_t ModbusMaster_ModbusMasterTransaction(uint8_t u8MBFunction)
{
	...
	while (u8BytesLeft && !u8MBStatus)
  {
    ...
    //lixichao add,in case of time overflow,reset u32StartTime,otherwise will loop here forever
    u32NowTime = Modbus_Master_Millis();
    if(u32StartTime > 0xF0000000 && u32NowTime < 0x10000000)
    {
      u32StartTime = u32NowTime;
    }
    
    if ((u32NowTime - u32StartTime) > ku16MBResponseTimeout)
    {
      u8MBStatus = ku8MBResponseTimedOut;
    }
    
    //lixichao add
    //增加一定的延时，不然Modbus_Master_Millis()函数里总是会频繁的关中断，导致进程运行不正常
    OSTimeDlyHMSM(0,0,0,10);
  }
