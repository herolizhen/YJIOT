...
{  
    ...
    if(L506_cmd_info.Is_wait_data == 1 || Is_http == 1 || mqtt_action != 0)
    {
        return;
    }
...
//此函数不需要太频繁的执行
void L506_fogcloud_init_and_check(void)
{
...
//    if(L506_register_net.Is_L506_registerd == 1 && is_disabled_cmd_echo == 0)
//    {
//        L506_Disnable_Command_Echo();
//    }
    
    if(L506_register_net.Is_L506_registerd == 1 && Is_Get_CCID == 0)
    {
        L506_Get_CCID();
    }
...
void L506_get_mqtt_connect_status_cmd(void)
{
    if(is_mqtt_inited == 0 || L506_cmd_info.Is_wait_data == 1 || Is_http == 1)
    {
        return;
    }
    
    mymemset(L506_cmd_info.cmd,0,sizeof(L506_cmd_info.cmd));
    mymemset(L506_cmd_info.sendtring,0,strlen(L506_cmd_info.sendtring));
...
{
//    if(mqtt_connect_status == 0 && is_mqtt_inited == 1)
//    {
//        L506_mqtt_close();
//    }
}
...
mqtt_init_start:  
    ...
    {
        //消息队列满了，push一个出来
        L506_LOG("mqtt init queue full,push one msg");
