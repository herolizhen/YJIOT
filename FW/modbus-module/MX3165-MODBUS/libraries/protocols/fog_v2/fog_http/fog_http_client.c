...
static OSStatus usergethostbyname( const char * domain, uint8_t * addr, uint8_t addrLen )
{
    ...
    //lxc test
    app_log("get out of gethostbyname");
...
extern unsigned char is_http_loop_fault;
//FOG V2 http client
static void fog_v2_http_client_thread(mico_thread_arg_t arg)
{
    OSStatus err = kGeneralErr;
 ...
 HTTP_SSL_START:
    ...
    //lxc add  用于监控是否进入usergethostbyname后不能跳出来
    is_http_loop_fault = 1;
    err = usergethostbyname(fog_host, (uint8_t *)ipstr, sizeof(ipstr));
    //lxc add
    is_http_loop_fault = 0;
    if ( err != kNoErr )
    {
