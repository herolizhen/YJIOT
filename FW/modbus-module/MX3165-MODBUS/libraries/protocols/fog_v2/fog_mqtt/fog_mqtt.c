...
mqtt_context_t *mqtt_context = NULL;
...
//lxc add
#ifdef USE_MODBOS_DEVICES   
    #define MODBUS_TOPIC_BUFF_SIZE  80
    uint8_t i = 0;
    char *p_modbus_topic = NULL;
    char *modbus_topic = NULL;
#endif
...
//lxc add
    ...
    while(modbus_devs.is_all_devs_activate == 0)
    {
        mico_thread_msleep(500);
    }
    
    p_modbus_topic = malloc(modbus_devs.dev_nums * MODBUS_TOPIC_BUFF_SIZE);
    if(p_modbus_topic == NULL)
    {
        app_log("MQTT thread malloc fail");
        goto MQTT_disconnect;
    }
    
    while(!modbus_devs.is_all_devs_sub)
    { 
        for(i = 0;i < modbus_devs.dev_nums;i++)
        {
            if(modbus_devs.devices[i]->is_sub == 0)
            {
                modbus_topic = p_modbus_topic + i * MODBUS_TOPIC_BUFF_SIZE;
                memset(modbus_topic,0,sizeof(MODBUS_TOPIC_BUFF_SIZE));
                sprintf( modbus_topic, "c2d/%s/commands", modbus_devs.devices[i]->device_id);
                app_log("%d device MQTT client subscribe commands",i);
                rc = MQTTSubscribe( &c, modbus_topic, QOS0, messageArrived );
                if ( MQTT_SUCCESS == rc )
                {
                    app_log("%d device MQTT client subscribe success! recv_topic=[%s].", i,modbus_topic);
                    modbus_devs.devices[i]->is_sub = 1;
                }
                else
                {
                    app_log("ERROR: %d device MQTT client subscribe err=%d.",i,rc);
                    goto MQTT_disconnect;
                }
                mico_thread_msleep(500);
                break;
        ...
        {
            modbus_devs.is_all_devs_sub = 1;
            app_log("all modbus devs sub");
        }
  }
...
OSStatus fog_v2_device_recv_command( char *payload, uint32_t payload_len, uint32_t timeout )
{
    ...
    if ( p_recv_msg )
    {
        ...
        app_log("[RECV] topic: %s", p_recv_msg->topic);
        app_log("[RECV] payload: %s", p_recv_msg->data);
