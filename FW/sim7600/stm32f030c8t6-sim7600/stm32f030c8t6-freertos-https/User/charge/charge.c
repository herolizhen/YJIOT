...
void charge_init(void)
{
    ...
    if(readbuff[0] == 0xFF)
    {
        ...
        mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
        sprintf(L506_cmd_info.sendtring,"way0\r\n");
        DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
...
//    readbuff[1] = EEPROM_Byte_Read(EEPROM_START_ADDR + 1);
    mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
    sprintf(L506_cmd_info.sendtring,"read data:%x %x %x %x %x %x\r\n",(unsigned int)readbuff[0],(unsigned int)readbuff[1],
    ...
    else
    {
        ...
        do{
              mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
              sprintf(L506_cmd_info.sendtring,"way3\r\n");
              DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
              //每次读取2组数据，10个字节
              mymemset(readbuff,0,sizeof(readbuff));
              ...
              {               
                  if(readbuff[0] == 0xFF)
                  {
                      ...
                      mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
                      sprintf(L506_cmd_info.sendtring,"way1\r\n");
                      DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
                  }
                  else
                  {
                      ...
                      mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
                      sprintf(L506_cmd_info.sendtring,"way2\r\n");
                      DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
                  }                    
              ...
              else if(eeprom_addr == (EEPROM_END_ADDR - 9))              
              {
                  ...
                  mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
                  sprintf(L506_cmd_info.sendtring,"page full,re-write\r\n");
                  DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
              ...
              else
              {
                  //肯定出错了
                  mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
                  sprintf(L506_cmd_info.sendtring,"eeprom addr out of range\r\n");
                  DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
...
void charge_lefttime_count(void)
{
    ...
    if(nowtime_s >= charge_info.next_1min_time)
    {
        ...
        mymemset(L506_cmd_info.sendtring,0,sizeof(L506_cmd_info.sendtring));
        sprintf(L506_cmd_info.sendtring,"lefttime:%d\r\n",(unsigned int)(charge_info.lefttime.l_time));
        DEBUG_Uart_Sendbytes(L506_cmd_info.sendtring,strlen(L506_cmd_info.sendtring));
